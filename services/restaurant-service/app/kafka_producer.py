...
def _delivery_callback(err, msg):
    if err:
        print(f"[restaurant-producer] delivery failed {msg.topic()}: {err}")
    else:
        print(f"[restaurant-producer] delivered to {msg.topic()} [{msg.partition()}]")
...
def _sync_publish(topic: str, payload: str) -> None:
    ...
    try:
        ...
        print(f"[restaurant-producer] kafka publish error (non-fatal): {exc}")
        _producer = None
...
async def publish_restaurant_event(topic: str, data: dict) -> None:
    ...
    try:
        ...
        print(f"[restaurant-producer] executor error (non-fatal): {exc}")
